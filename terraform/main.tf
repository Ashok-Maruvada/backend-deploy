## while doing this(backend configuration), VPN is must enabled -to connect backend server via ssh to do the configuration
module "backend" {
...
resource "null_resource" "backend" {
  triggers = {
    instance_ids = module.backend.id # it will trigger whenever instance is created
  }
  connection {
        type     = "ssh"
        user     = "ec2-user"
  ...
  provisioner "file" {
   ...
   provisioner "remote-exec" {
    inline = [
      ...
      "sudo sh /tmp/backend.sh ${var.comman_tags.component} ${var.environment} ${var.app_Version} "
    ]
  }
}
...
# u can browse backend application via backend.app-dev.goadd.fun 
